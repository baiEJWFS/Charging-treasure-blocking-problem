...
parser.add_argument('--save_folder', default=val_save_folder, type=str, help='File path to save results')
parser.add_argument('--confidence_threshold', default=confidence_threshold, type=float,
...
# annopath = os.path.join(args.sixray_root, 'core_3000', 'Annotation', '%s.txt')
annopath = os.path.join(args.sixray_root, 'Anno_core_coreless_battery_sub_2000_500', '%s.txt')
# imgpath = os.path.join(args.sixray_root, 'core_3000', 'Image', '%s.jpg')
imgpath = os.path.join(args.sixray_root, 'cut_Image_core_coreless_battery_sub_2000_500', '%s.jpg')

# imgsetpath = os.path.join(args.sixray_root, 'core_3000', 'ImageSets', 'Main', '{:s}_core_3000.txt')
...
def parse_rec(filename, imgpath):
    ...
    imagename0 = filename.replace('Anno_core_coreless_battery_sub_2000_500',
                                  'cut_Image_core_coreless_battery_sub_2000_500')
    imagename1 = imagename0.replace('.txt', '.jpg')
    objects = []
    img = cv2.imread(imagename1)
    height, width, channels = img.shape
...
"""
    ...
    if any(lines) == 1:
        ...
        # print('===============', splitlines)
        # image_ids = [x[1][1:-2] for x in splitlines]
        ...
        # print('333333333',image_ids)
        # go down dets and mark TPs and FPs
        nd = len(image_ids)
        tp = np.zeros(nd)
        fp = np.zeros(nd)
        for d in range(nd):
            R = class_recs[str(image_ids[d])]
...
def reset_args(EPOCH):
    ...
    #     EPOCH)
    # save_folder = '/media/trs2/wuzhangjie/SSD/eval/Xray20190723/Attention/base_battery_core_coreless_bs8_V/'
    if not os.path.exists(args.save_folder):
        os.mkdir(args.save_folder)
    args.save_folder = args.save_folder + '{:d}epoch_500/'.format(EPOCH)
